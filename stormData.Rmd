...
```
## 1.Overview  
In this report, we analyze the NOAA Storm Database to answer the questions about the damages that severe weather events caused across the United States. Namely,
...
To answer these questions, We will find out which severe weather events are attributed to higher fatalities/injuries or larger damages to the crops/properties, so that readers can conduct further analyses and take appropriate prevention measures for those events.

## 2.Data Processing  

In this section, we load the raw data, subset and alter some columns to make the data ready for analyses.  
(*I set echo=TRUE for all the code for grading purpose. This section may be a little lengthy.)
...
Now after changed them to all upper cases, unique EVTYPE values are reduced to `r after` from `r before`.  
...
kbl(matrix(events, nrow=16,dimnames =NULL), caption='Official Event Types (Table.1)',
...
before <- dim(data)[1]
# Count unique EVTYPE each year (This code needed for appendix1)
grouped <- data %>%
...
And lastly, make total columns of damages to make an analysis easier.  
```{r newColumn}
# make new columns for total of health/economic damages
data <- mutate(data, 
               totFatInj=FATALITIES+INJURIES,totCropProp=totPropDmg+totCropDmg)

```


## 3.Results
Table.2 is a list of top 5 weather events which caused the highest fatalities and injuries on US citizens from 1993 to 2011. (see full list for appendix 2).  
...
Rank <- 1:5
kbl(cbind(Rank,healthDmgSum[1:5,]), caption='Top 5 events Recorded Highest  Fatalities/Injurie(Descending order of Total) Table.2',
             type='html')  %>% 
...
### Findings for severe weather events wchich are most harmful to population health
...
* TORNADO has the highest figure of fatalities and injuries in the records. From 1993, mostly keep the highest place, and 2011 was the one of the worst year, `r pctTornado`% of the total fatalities/injuries in this year were caused by TORNADO.  

* The EXCESSIVE HEAT has the highest fatalities among all the events. 

*  The number of the fatalities/injuries by FLOOD were relatively low except 1998. The figure in this year occupies nearly `r pct1998`% of the total fatalities/injuries in the record.  

* THUNDERSTORM WIND and LIGHTNING have similar trend through the record.


```{r economyResult}
economicDmgSum <- data %>%
  group_by(EVTYPE) %>%
  summarize(cropSum=sum(totCropDmg),propSum=sum(totPropDmg),Total=sum(totCropProp)) %>%
  arrange(desc(Total))

Rank <- 1:5
kbl(cbind(Rank,economicDmgSum[1:5,]), caption='Table.3 Top 5 events Recorded Highest Damages to Crops/Properties (Descending order of Total)',
             type='html')  %>% 
  kable_styling(latex_options = 'hold_position',full_width=F,position='left') %>%
  column_spec(2,bold=T)%>%
  column_spec(5,bold=T)
...
```
```{r economicDmgFindngs}

flood <- filter(data, EVTYPE=='FLOOD')
pctFlood2006 <- round(
            sum(flood$totCropProp[flood$YEAR=='2006'])/sum(flood$totCropProp),2)*100
library(reshape2)
tmp <- dcast(PlotEconomy,YEAR ~ EVTYPE,value.var='YEARLY')
colSums(is.na(tmp))
rownames(tmp) <- tmp$YEAR
tmp <- select(tmp, -c(1))
tmp
c <- round(cor(tmp, use = 'complete.obs'),2)
c
```

The weather event that have the greatest economic consequences is FLOOD. From the Figure.1, Flood is not always has the greatest damage, this high figure is due to the 2006 record which is `r pctFlood2006`% of all the damages the Flood caused. 

The HURRICANE (TYPHOON) and STORM SURGE/TIDE have almost same trend every year.  
